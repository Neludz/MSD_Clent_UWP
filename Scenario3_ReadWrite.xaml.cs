...
namespace CustomSerialDeviceAccess
{
    ...
    public sealed partial class Scenario3_ReadWrite : Page, IDisposable
    {
        ...
        async private void Start_Write()
        {
        ...
        async private void Start_Read()
        {
            if (EventHandlerForDevice.Current.IsDeviceConnected)
            {
                try
                {
                    ...
                    EventHandlerForDevice.Current.Device.ReadTimeout = new System.TimeSpan(100 * 10000);
        ...
        private async void WriteButton_Click(object sender, RoutedEventArgs e)
        {
            if (EventHandlerForDevice.Current.IsDeviceConnected)
            {
                try
                {
                    ...
                    var stateTimer = new Timer(TimerProc,null,5000, 0);
        ...
        private  void TimerProc(object state)
        {
            ...
            Debug.WriteLine("TTTTTTTTTIIIIIIIIIIIIMMMMMMMMMMMMMMEEERRRRRRRRRR");
           //// await Window.Current.CoreWindow.Dispatcher.RunAsync(Windows.UI.Core.CoreDispatcherPriority.Normal,
           //  () => {
               permission = false;
                    ReadCancelButton_Click(null, null);
              //      Timer t = (Timer)state;
                //    t.Dispose();
        ...
        */
        ...
        private async Task ReadAsync(CancellationToken cancellationToken)
        {
            ...
            byte[] iv;
           // while (true && EventHandlerForDevice.Current.IsDeviceConnected && permission)
          //  {
                ...
                Debug.WriteLine($"STAR READ  T!!!!!!!!!!!!_____{bytesRead}");
                if (bytesRead > 0)
                {
                   
                    iv = new byte[bytesRead];
                    DataReaderObject.ReadBytes(iv);
                    // ReadBytesTextBlock.Text += DataReaderObject.ReadString(bytesRead);
                    ReadBytesTextBlock.Text += BitConverter.ToString(iv);
        ...
        private async Task WriteAsync(CancellationToken cancellationToken)
        {
            ...
            byte[] Buf = new byte[] { 0x01, 0x03, 0x00, 0x00, 0x00, 0x0A, 0xC5, 0xCD };

           // while (EventHandlerForDevice.Current.IsDeviceConnected && permission)
           // {
